...
async function storeAsset(file: string, slug: string) {
  const filePath = path.join(assetsDir(), file);
  if (!(await fileExists(filePath))) {
    throw new Error(`Missing blog asset: ${file}`);
  }
  const ext = path.extname(file) || ".jpg";
  return storeImageFromPath(filePath, slug, "blogs", `${slug}${ext}`);
}
...
export async function seedBlogs(): Promise<void> {
  ...
  for (const [index, item] of DEFAULTS.entries()) {
    const main = await storeAsset(item.image, item.slug);
    docs.push({
      slug: item.slug,
      title: item.title,
      ...
      imageAlt: "imageAlt" in item ? item.imageAlt : "",
      projectSlug: item.projectSlug,
      imageUrl: main.imageUrl,
      imagePublicId: main.imagePublicId,
      body: await Promise.all(
        item.body.map(async (section, sectionIndex) => ({
          heading: section.heading,
          text: section.text,
          points: await Promise.all(
            section.points.map(async (point, pointIndex) => {
              const imageName = "image" in point ? String(point.image || "") : "";
              let imageUrl = "";
              let imagePublicId = "";
              if (imageName) {
                const stored = await storeAsset(imageName, `${item.slug}-point-${sectionIndex + 1}-${pointIndex + 1}`);
  ...
  console.log("Blog articles ready");
}
...
async function resolveSource(imageUrl: string, slug: string) {
  ...
  const original = DEFAULTS.find((item) => item.slug === slug || imageUrl.includes(`/${item.slug}`));
  if (!original) return "";
  const assetPath = path.join(assetsDir(), original.image);
  return (await fileExists(assetPath)) ? assetPath : "";
}

export async function migrateBlogImagesToCloudinary(): Promise<void> {
  try {
    await getCloudinary().api.ping();
  } catch (err) {
    console.error("Cloudinary ping failed. Blog images were not moved.", err);
    return;
  }
  ...
  for (const doc of docs) {
    const source = await resolveSource(doc.imageUrl, doc.slug);
    if (!source) continue;
    const uploaded = await storeImageFromPath(source, doc.slug, "blogs", `${doc.slug}.jpg`);
    if (uploaded.imageUrl === doc.imageUrl) continue;
  ...
  if (moved > 0) console.log(`Moved ${moved} blog image(s) to Cloudinary`);
}

const POINT_IMAGES: Record<string, string> = {
  "Anti-skid flooring": "why-materials.jpg",
  "Space for towels and seating": "floorplan.jpg",
  "Hotel gym with large windows overlooking a natural landscape": "gallery-1.jpg",
  "Hotel gym with planned equipment spacing and clear circulation": "gym-layout.jpg",
  "Hotel gym interior with layered architectural lighting": "case-study.jpg",
  "Modern gym interior with multiple training zones": "project-6.jpg",
  "Gym interior combining natural and architectural lighting": "project-4.jpg",
  "Boutique gym interior with clear equipment spacing and circulation": "project-3.jpg",
  "Wellness and recovery space within a boutique fitness environment": "project-5.jpg",
  "Durable rubber flooring in a gym strength training area": "project-1.jpg",
  "Gym flooring material detail showing texture and floor transition": "why-materials.jpg",
  "Gym interior with clear circulation and negative space between equipment": "project-3.jpg",
  "Gym interior with layered architectural lighting": "project-4.jpg",
};

export async function attachBlogPointImages(): Promise<void> {
  const docs = await Blog.find();
  let attached = 0;
  ...
  for (const doc of docs) {
    let changed = false;
    const body = [];
    for (const [sectionIndex, section] of doc.body.entries()) {
      const points = [];
      for (const [pointIndex, point] of section.points.entries()) {
        const next = {
          ...
          imagePublicId: point.imagePublicId || "",
        };
        const asset = POINT_IMAGES[point.heading];
        if (asset && !next.imageUrl) {
          const stored = await storeAsset(asset, `${doc.slug}-point-${sectionIndex + 1}-${pointIndex + 1}`);
          next.imageUrl = stored.imageUrl;
          next.imagePublicId = stored.imagePublicId;
          changed = true;
          attached += 1;
        }
        points.push(next);
      }
      body.push({ heading: section.heading, text: section.text, points });
    }
    if (changed) {
      await Blog.updateOne({ _id: doc._id }, { $set: { body } });
    }
  }

  if (attached > 0) console.log(`Attached ${attached} blog point image(s)`);
}

async function applyDefaultToBlog(
  item: (typeof DEFAULTS)[number],
  sortOrder: number,
  extraSlugs: string[],
) {
  const doc = await Blog.findOne({
    $or: [{ sortOrder }, { slug: item.slug }, ...extraSlugs.map((slug) => ({ slug }))],
  }).sort({ sortOrder: 1 });

  if (!doc) {
    throw new Error(`Blog card ${sortOrder} was not found`);
  }
        ...
        section.points.map(async (point, pointIndex) => {
          ...
          if (imageName) {
            const stored = await storeAsset(
              imageName,
              `${item.slug}-point-${sectionIndex + 1}-${pointIndex + 1}`,
            );
            imageUrl = stored.imageUrl;
            imagePublicId = stored.imagePublicId;
          }
  ...
  doc.readTime = item.readTime;
  doc.excerpt = item.excerpt;
  doc.highlight = "highlight" in item ? item.highlight : "";
  doc.imageAlt = "imageAlt" in item ? item.imageAlt : "";
  doc.projectSlug = item.projectSlug;
  doc.imageUrl = main.imageUrl;
  doc.imagePublicId = main.imagePublicId;
  doc.body = body;
  doc.sortOrder = sortOrder;
  await doc.save();
  console.log(`Updated blog ${sortOrder}: ${doc.slug}`);
}

export async function replaceSecondBlogWithHospitalityArticle(): Promise<void> {
  await applyDefaultToBlog(DEFAULTS[1], 2, ["how-equipment-placement-affects-user-experience"]);
}

export async function replaceThirdBlogWithEvolutionArticle(): Promise<void> {
  await applyDefaultToBlog(DEFAULTS[2], 3, ["gym-mirrors-and-lighting-need-different-planning"]);
}

export async function replaceFourthBlogWithBoutiqueArticle(): Promise<void> {
  await applyDefaultToBlog(DEFAULTS[3], 4, ["balancing-aesthetics-with-maintenance"]);
}

export async function replaceFifthBlogWithPremiumArticle(): Promise<void> {
  await applyDefaultToBlog(DEFAULTS[4], 5, ["what-designing-multiple-gyms-taught-us"]);
}

export async function replaceSixthBlogWithFlooringArticle(): Promise<void> {
  await applyDefaultToBlog(DEFAULTS[5], 6, ["wrong-assumptions-about-gym-design"]);
}
